...
public class ObserverPatternExample {
	...
	// and its observers
		Sales salseObj = new Sales();
		Table table = new Table();
		PieChart pieChartObj = new PieChart();
		BarChart barChartObj = new BarChart();
		
		salseObj.addObserver(table);
		salseObj.addObserver(pieChartObj);
		salseObj.addObserver(barChartObj);
 ...
 // The update operation is missing some code
  ...
  public void update(Observable obj, Object arg) {
	  if(arg instanceof Sales) {
		  Sales salseObj = (Sales) arg;
		  int[] values = salseObj.getValues();
	      for (int i=0; i<5; i++) 
